...
def remap(indata):
    newdata = float(indata-1500)/1000
    return int(newdata*255.0+255/2)
...
def rcUnpack(packedData):
    ...
    device.emit(uinput.ABS_X, remap(int(channels[0])), syn=False) #throttle
    device.emit(uinput.ABS_Y, remap(int(channels[3]))) #yaw
    device.emit(uinput.ABS_RX, remap(int(channels[1])), syn=False) #roll
    device.emit(uinput.ABS_RY, 255-remap(int(channels[2]))) #pitch
